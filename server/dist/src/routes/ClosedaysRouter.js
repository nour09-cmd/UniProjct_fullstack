...
const CloseDaysController_1 = require("../controller/CloseDaysController");
const authenticateTokenAndCheckRole_1 = require("../middlewares/authenticateTokenAndCheckRole");
const router = (0, express_1.Router)();
const closeDaysController = new CloseDaysController_1.CloseDaysController();
...
router
    ...
    try {
        yield closeDaysController.getCloseDays(req, res);
    }
    catch (error) {
    ...
    try {
        yield closeDaysController.createCloseDays(req, res);
    }
    catch (error) {
        res.status(500).json({ message: "Internal server error" });
    }
    ...
    try {
        yield closeDaysController.deleteCloseDays(req, res);
    }
    catch (error) {
        res.status(500).json({ message: "Internal server error" });
    }
...
//# sourceMappingURL=ClosedaysRouter.js.map